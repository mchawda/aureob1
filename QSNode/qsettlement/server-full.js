...
app.post('/api/v1/mint', async (req, res) => {
  try {
    ...
    const tx = await fiatToken.mint(
      to,
      ethers.parseEther(amount.toString()),
      ethers.id(reference) // Use ethers.id instead of formatBytes32String
    );
...
app.post('/api/v1/burn', async (req, res) => {
  try {
    ...
    // Check balance
    const balance = await fiatToken.balanceOf(from);
...
app.post('/api/v1/transfer', async (req, res) => {
  try {
    ...
    // Execute transfer
    const tx = await fiatToken.transfer(to, ethers.parseEther(amount.toString()));
    const receipt = await tx.wait();
...
app.get('/api/v1/reserves/:currency', async (req, res) => {
  try {
    ...
    res.json({
      ...
      status: {
        healthy: isHealthy,
        ratio: ratio.toNumber() / 100, // Convert from basis points
        lastUpdate: reserveData.lastAttestationTime.toNumber(),
        issues: isHealthy ? [] : ['Reserve not healthy']
      },
      timestamp: new Date().toISOString()
    });
...
app.get('/api/v1/compliance/:address', async (req, res) => {
  try {
    ...
    const isWhitelisted = await complianceGate.whitelistedAddresses(address);
    const isBlacklisted = await complianceGate.blacklistedAddresses(address);
    const dailyLimit = await complianceGate.dailyLimits(address);
    const transactionLimit = await complianceGate.transactionLimits(address);
    
    res.json({
      address,
      compliance: {
        kycVerified: isWhitelisted,
        kycLevel: isWhitelisted ? 'enhanced' : 'basic',
        riskScore: isBlacklisted ? 100 : (isWhitelisted ? 25 : 50),
        sanctionsPassed: !isBlacklisted,
        dailyLimit: dailyLimit.toString(),
        transactionLimit: transactionLimit.toString(),
        restrictions: isBlacklisted ? ['Address is blacklisted'] : [],
        lastCheck: Date.now()
      },
      timestamp: new Date().toISOString()
    });
...
app.post('/api/v1/fees/calculate', async (req, res) => {
  try {
    ...
    const feeAmount = await feeRouter.calculateFee(user, ethers.parseEther(amount.toString()));
    ...
    res.json({
      ...
      feeRate: (feeAmount * 10000n) / ethers.parseEther(amount.toString()), // basis points
      timestamp: new Date().toISOString()
    });
